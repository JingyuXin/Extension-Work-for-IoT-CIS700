...
if __name__ == '__main__':
    ...
    net.load_state_dict(torch.load(modelpath + 'Best_model.pth'))
    ...
    for file in testFiles:
        ...
        for j in range(10):
            ...
            for i in range(num_sample):
                purelst.append(x_pair(template, Xpure[i, :, :], 1))
                syn100lst.append(x_pair(template, Xsyn100[i, :, :], 0))
                syn200lst.append(x_pair(template, Xsyn200[i, :, :], 0))
                syn300lst.append(x_pair(template, Xsyn300[i, :, :], 0))
                syn400lst.append(x_pair(template, Xsyn400[i, :, :], 0))
                syn500lst.append(x_pair(template, Xsyn500[i, :, :], 0))
                imposterlst.append(x_pair(template, Ximposter[i, :, :], 0))

            accPure = test(purelst, net)[3]
            acc100 = test(syn100lst, net)[3]
            acc200 = test(syn200lst, net)[3]
            acc300 = test(syn300lst, net)[3]
            acc400 = test(syn400lst, net)[3]
            acc500 = test(syn500lst, net)[3]
            accImposter = test(imposterlst, net)[3]
        ...
        f = open(modelpath + 'Phase2_best_res.txt', 'w')
        f.write(str(res))
        f.close()
