...
# This file generates data pairs for phase 1 training of Siamese Network
...
def samplePairs(files, num, datapath, size):
    count = 0
    X = {'same': [], 'diff': []}
    while (count < num):
        ...
        s11 = sample(file1, size).T
        s12 = sample(file1, size).T
        s21 = sample(file2, size).T
        s22 = sample(file2, size).T
...
if __name__ == '__main__':
    ...
    X_train = samplePairs(trainFiles, 2000, datapath, 1000)
    X_val = samplePairs(valFiles, 300, datapath, 1000)
    X_test = samplePairs(testFiles, 500, datapath, 1000)

    out = '/home/jxin05/10sPhase1/'

    with open(out + 'train.pickle', 'wb') as f:
        pickle.dump(X_train, f, protocol=pickle.HIGHEST_PROTOCOL)
    with open(out+'test.pickle', 'wb') as f:
        pickle.dump(X_test, f, protocol=pickle.HIGHEST_PROTOCOL)
    with open(out+'validation.pickle', 'wb') as f:
